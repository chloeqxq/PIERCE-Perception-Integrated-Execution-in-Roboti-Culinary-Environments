...
DEVICENAME                  = '/dev/serial/by-id/usb-1a86_USB_Single_Serial_5A7A058163-if00'    # Check which port is being used on your controller
...
for id in SCS_IDS:
    ...
    scs_addparam_result = groupSyncRead.addParam(id)
...
while 1:
    ...
    while 1:
        ...
        for id in SCS_IDS:
            scs_getdata_result = groupSyncRead.isAvailable(id, ADDR_STS_PRESENT_POSITION, 4)
            scs_present_position_speed = 0
            if scs_getdata_result == True:
                # Get SCServo#1 present position value
                scs_present_position_speed = groupSyncRead.getData(id, ADDR_STS_PRESENT_POSITION, 4)
            else:
                print("[ID:%03d] groupSyncRead getdata failed" % id)


            scs1_present_position = SCS_LOWORD(scs_present_position_speed)
            scs1_present_speed = SCS_HIWORD(scs_present_position_speed)
            print("[ID:%03d] PresPos:%03d PresSpd:%03d" 
                % (id,  scs1_present_position, SCS_TOHOST(scs1_present_speed, 15), ),end=" ")
        print()
